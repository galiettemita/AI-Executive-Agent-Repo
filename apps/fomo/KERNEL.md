...
| **3D.2** | Slack Approval Capture + Slack Smoke Test — inbound Slack webhook with HMAC signature verification, captures approve/reject reactions or interactive components, transitions alert `queued_for_review → approved | rejected`. Founder smoke gate proves the full alert lifecycle end-to-end against a real Slack workspace. **Next gate.** |
