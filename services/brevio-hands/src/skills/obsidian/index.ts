...
const adapter: ISkillAdapter = {
  ...
  outputSchema: {
    ...
    properties: {
      provider: { type: 'string', enum: ['obsidian'] },
      action: { type: 'string', enum: ['list', 'create', 'search', 'update'] },
      note_id: { type: 'string' },
      notes: { type: 'array' }
    },
  ...
  async execute(input: SkillInput, _ctx: SkillContext): Promise<SkillResult> {
    ...
    try {
      const data = await runClient(parsed.data as OBSIDIANInput);
      const output = OutputSchema.parse(data) as OBSIDIANOutput;
      return {
        skill_id: 'obsidian',
        status: 'SUCCESS',
        data: output,
      ...
      const validationError =
        ...
        (err.message === 'OBSIDIAN_CREATE_FIELDS_REQUIRED' ||
          err.message === 'OBSIDIAN_UPDATE_FIELDS_REQUIRED');
