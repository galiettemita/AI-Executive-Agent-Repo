...
- [ ] **Founder has executed the smoke eval and committed `docs/OPENAI_SMOKE_REPORT_3C2.md` + `docs/openai-smoke-eval-3c2-results.json` to the SAME PR branch (`phase3c2-openai-ranker-smoke-eval`), then merged. Verdict must be `PASS` OR `INVESTIGATE` with a documented plan.** (load-bearing — Phase 3C.3 may NOT begin until checked. The PR stays open through the founder's smoke eval; "scaffolding committed" is not "phase complete.")
...
| **Gmail Polling Worker** (Phase 3B.2) | [src/workers/gmail-poll.ts](src/workers/gmail-poll.ts) | [workers/gmail-poll.test.ts](src/workers/gmail-poll.test.ts) | ~14 | `apps/fomo/src/index.ts` (when `FOMO_GMAIL_POLLING_ENABLED=true`); integration harness (runs one cycle) | Phase 3C.3 plugs the ranker as the downstream consumer of dispatched message bodies |
...
| **OpenAI backend** (NEW Phase 3C.2) | [src/core/model-backends/openai.ts](src/core/model-backends/openai.ts) | [core/model-backends/openai.test.ts](src/core/model-backends/openai.test.ts) | ~16 | OpenAI smoke eval script (`scripts/smoke-eval-3c2.ts`); registered by 3C.3 caller for the polling worker | Phase 3C.3 registers it on the model router |
| **FOMO Ranker** (Phase 3C.1) | [src/ranker/index.ts](src/ranker/index.ts) + [prompt.ts](src/ranker/prompt.ts) + [validator.ts](src/ranker/validator.ts) | [ranker/index.test.ts](src/ranker/index.test.ts) + [validator.test.ts](src/ranker/validator.test.ts) | ~27 | (pure function; no production callers in 3C.1 or 3C.2) | Phase 3C.3 calls it from the polling worker after `gmail.read` dispatch |
...
| 3C.3 | Wire OpenAI ranker into the polling worker behind `FOMO_RANKER_ENABLED` kill switch (default false), using the model picked in 3C.2 |
