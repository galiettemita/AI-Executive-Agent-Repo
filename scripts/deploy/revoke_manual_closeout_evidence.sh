...
if ! command -v python3 >/dev/null 2>&1; then
  echo "python3 is required to revoke manual closeout evidence" >&2
  exit 1
fi
...
if [[ -z "$ITEM_ID" ]]; then
  echo "usage: revoke_manual_closeout_evidence.sh <item_id> <revoked_by> [note]" >&2
  exit 1
fi

if [[ -z "$REVOKED_BY" ]]; then
  echo "revoked_by is required" >&2
  exit 1
fi
...
python3 - "$MANUAL_EVIDENCE_PATH" "$ITEM_ID" "$REVOKED_BY" "$NOTE" <<'PY'
import json
import os
import sys
from datetime import datetime, timezone

path, item_id, revoked_by, note = sys.argv[1:5]
...
print(json.dumps({"item_id": item_id, "status": "revoked", "path": path}, indent=2))
PY
