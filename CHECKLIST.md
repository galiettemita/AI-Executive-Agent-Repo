...
- [x] BREVIO x OPENCLAW EKS auth hardening: added explicit AWS credential bootstrap to deploy workflows (`AWS_ROLE_TO_ASSUME` OIDC first, static key fallback) and kubeconfig exec-auth preflight (`aws sts get-caller-identity`) so EKS token-based kubeconfigs do not fail with `NoCredentials` on GitHub runners.
- [x] BREVIO x OPENCLAW workflow queueing hardening: replaced invalid `secrets.*` references inside workflow-step `if` expressions with job-level `env` projections (`env.*`) to restore valid workflow_dispatch queueing and prevent "Failed to queue workflow run" parser failures.
...
- [x] Strict closure hardening: added source comment hygiene gate (`internal/contracts/comment_hygiene_closure_test.go`) to continuously enforce no unresolved action-item or deprecation markers in implementation artifacts
...
- [x] Strict closure hardening: upgraded Stage 6 security gates to explicitly run Semgrep plus runtime security validation in `ci-openclaw` and `security-scan` workflows; prior Snyk-only enforcement was later superseded by the security gate policy override.
- [x] Strict closure hardening: replaced generic skill-registry seeded descriptions/use-cases by adding a deterministic 153-skill metadata normalization block in `migrations/006_seed_skills.up.sql` keyed by seeded skill IDs.
- [x] Security gate policy override closure: removed the hard `SNYK_TOKEN` requirement from `ci-openclaw` and `security-scan` workflows, keeping Semgrep, `pnpm audit`, Trivy, TruffleHog, Syft, and govulncheck as the enforced security stack per current operating choice.
...
- [x] Production deployment execution-checklist phase closure: added `make production-deployment-checklist` to generate `artifacts/deploy/production_deployment_checklist.md` from the signoff artifact with explicit rollout/canary/rollback/evidence steps for immediate runbook execution.
...
- [x] Workflow queue/lint hardening closure: removed invalid `secrets.*` condition usage from dispatch workflows (replaced with env-projected checks), fixed `llm-evals` artifact selection script to satisfy ShellCheck (`find|sort|tail`), and revalidated all workflow files with `actionlint` (clean pass).
- [x] Legacy `ci` workflow hardening closure: added the missing `gopkg.in/check.v1` checksum entries so `go mod tidy check` remains clean on GitHub runners, and made the HashiCorp Terraform unzip step non-interactive (`unzip -oq`) so preinstalled runner binaries do not block the workflow with `replace terraform?` prompts.
