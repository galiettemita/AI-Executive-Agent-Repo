...
- [x] Strict closure hardening: upgraded Stage 6 security gates to explicitly run Semgrep + Snyk + runtime security validation in `ci-openclaw` and `security-scan` workflows, with strict `SNYK_TOKEN` requirement and artifact persistence for `artifacts/security/snyk_report.json`.
