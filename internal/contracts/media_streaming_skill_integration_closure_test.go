...
func TestMediaStreamingSkillIntegrationFixturesClosure(t *testing.T) {
	t.Parallel()
	...
	for _, skillID := range skillIDs {
		...
		if err != nil {
			t.Fatalf("read media integration test for %s: %v", skillID, err)
		}
		if strings.Contains(string(integrationBody), "scaffold compiles") {
			t.Fatalf("media integration test still scaffold-only for %s", skillID)
		}
		...
		if jsonCount == 0 {
			t.Fatalf("missing fixture json for media skill %s", skillID)
		}
	}
}
