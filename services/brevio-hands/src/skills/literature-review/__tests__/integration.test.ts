...
describe('literature-review integration', () => {
  it('returns deterministic literature scan payload', async () => {
    const result = await adapter.execute(
      { action: 'search_papers', topic: 'agentic orchestration' },
      {} as never
    );
    ...
    assert.equal(result.data?.papers?.length, expected.papers_count);
  });
});
