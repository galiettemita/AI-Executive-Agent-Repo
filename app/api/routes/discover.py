...
async def search(q: str):
    try:
        ...
        # SerpAPI returned a non-2xx status (401/403/429/etc)
        status = e.response.status_code
        body = e.response.text[:500]  # avoid giant payloads
        raise HTTPException(status_code=502, detail=f"SerpAPI HTTP {status}: {body}")

    except httpx.RequestError as e:
        # network issue, DNS, timeout, TLS, etc.
        raise HTTPException(status_code=502, detail=f"Network error calling SerpAPI: {str(e)}")
