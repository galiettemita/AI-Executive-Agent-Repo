...
- [ ] **Founder has executed the demo end-to-end against real Gmail + real OpenAI + real Slack + real SendBlue and committed `docs/SMOKE_REPORT_3G.md` with `VERDICT: PASS`** (load-bearing — v0.1 milestone is declared done when this lands on `main`; v0.5 friend beta gated behind it)
...
| **3G** | **Full Founder Demo Smoke Test (v0.1 milestone).** ONE happy-path email flows end-to-end against real Gmail + OpenAI + Slack + SendBlue (outbound + inbound). Founder reply IN scope. Scaffolding live on `phase3g-full-demo-smoke`. v0.5 friend beta gated behind 3G PASS landing on `main` |
